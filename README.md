...
- `tg:registerAccrual(id, spec)` — `spec = { cadence = "day"|"month"|"year", flowClass = "calendar"|"usage"|"event"|"simulation", firstPeriodPolicy = "prorate"|"full"|"skip", priority = number, onSettle = function(ctx) ... end }`. `cadence` is required; a missing or unrecognized value is rejected, `registerAccrual` returns `false`, and nothing is registered (no silent daily default). The callback runs server-only, once per crossed boundary, and does its own money write. `simulation` is for non-money state evolution (day-cadence work that must survive skipped days with no forfeit); nothing that writes money may ever register under it.
