...
g_currentMission = {
  time = 1000,
  environment = { currentDay = 1, daysPerPeriod = 1, currentMonotonicDay = 1 },
  missionInfo = {},
}
...
-- Logging: TGLogger forwards to the engine's Logging.* globals. The stubs also
-- record every call so a test can assert that a warning was (or was not) raised.
LogCapture = { info = {}, warning = {}, error = {} }
local function _record(bucket)
  return function(msg, ...)
    local ok, formatted = pcall(string.format, msg, ...)
    bucket[#bucket + 1] = ok and formatted or tostring(msg)
  end
end
Logging = {
  info    = _record(LogCapture.info),
  warning = _record(LogCapture.warning),
  error   = _record(LogCapture.error),
}
function LogCapture.reset()
  LogCapture.info, LogCapture.warning, LogCapture.error = {}, {}, {}
  Logging.info    = _record(LogCapture.info)
  Logging.warning = _record(LogCapture.warning)
  Logging.error   = _record(LogCapture.error)
end
